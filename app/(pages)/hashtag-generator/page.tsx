...
import { HashtagIcon, SparklesIcon } from "@heroicons/react/24/outline";
...
export default function HashtagGeneratorPage() {
  ...
  const handleSubmit = async () => {
    if (status === "unauthenticated" || !session) {
      router.push(
        `/signup?callbackUrl=${encodeURIComponent(pathname || "/hashtag-generator")}`,
      );
      return;
    }
    ...
    if (!prompt.trim()) {
      setError("Please enter a topic or post description.");
      return;
    }
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            Enter your post topic or niche to get relevant hashtags for Instagram, TikTok, or Twitter.
          </p>
        </div>

        <div className="space-y-2">
          <label htmlFor="prompt" className="text-sm font-medium text-foreground">
            Topic or post description
          </label>
          <textarea
            id="prompt"
            ...
            placeholder="e.g. Vegan breakfast recipe, fitness motivation, travel in Japan"
            value={prompt}
        ...
        {result && (
          <div className="space-y-2">
            ...
            <div className="w-full min-h-[80px] rounded-lg border border-input bg-muted/50 p-4 text-sm text-foreground whitespace-pre-wrap">
              {result}
            </div>
          </div>
        )}
