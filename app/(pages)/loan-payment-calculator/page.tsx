...
export default function LoanPaymentCalculatorPage() {
  ...
  const handleCalculate = () => {
    if (status === "unauthenticated" || !session) {
      router.push(
        `/signup?callbackUrl=${encodeURIComponent(pathname || "/loan-payment-calculator")}`,
      );
      return;
    }
    ...
    <DashboardLayout>
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Loan Payment Calculator</h1>
        ...
        <div className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/40 p-6 sm:p-8 space-y-6">
          ...
          <div className="flex gap-4 pt-4">
            <Button
              ...
              disabled={!loanAmount || !apr || !termYears}
              className="gap-2"
            >
          ...
          {result && resultUnlocked && (
            <div className="pt-6 border-t border-slate-200 dark:border-slate-700 space-y-4">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Over {result.months} months. Estimate only; does not include fees or insurance.
              </p>
            </div>
          )}
        </div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-6">
          Formula: M = P × [r(1+r)^n] / [(1+r)^n − 1], where P = principal, r = monthly rate, n =
          number of payments.
        </p>
      </div>
    </DashboardLayout>
