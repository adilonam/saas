...
export async function POST(request: Request) {
  try {
    ...
    const outputFormatFromRequest =
      typeof outputFormatRaw === "string" ? outputFormatRaw.trim() : "";
    const outputFormat = outputFormatFromRequest || "jpeg";
    ...
    const upstreamResponse = await fetch(
      `${fastApiUrl}/fast-api/v1/compress-image`,
      {
        method: "POST",
    ...
    if (!upstreamResponse.ok) {
        ...
        return NextResponse.json(
          {
            error:
              ...
              "Failed to compress image on backend",
          },
      ...
      return NextResponse.json(
        { error: errorText || "Failed to compress image on backend" },
        { status: upstreamResponse.status }
      );
    }
