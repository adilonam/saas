...
import {
  calculateEqResultFromAnswerKey,
  clientAnswersToSelected,
  type SelectedAnswer,
} from "@/lib/eq-test/scoring";
...
export async function POST(request: Request) {
  try {
    ...
    if (attemptId) {
      const attempt = await prisma.eqTestAttempt.findUnique({
        where: { id: attemptId },
      });
      if (!attempt) {
        return NextResponse.json(
    ...
    if (!selected) {
      return NextResponse.json(
        {
          error:
            "Invalid answers. Expected answers: Array<{ question, selectedOption }> with 1-based selectedOption (1–4).",
        },
        { status: 400 },
      );
    }
    ...
    const result = calculateEqResultFromAnswerKey(
      selected,
    ...
    if (attemptId) {
      const updated = await prisma.eqTestAttempt.update({
        ...
        } satisfies Prisma.EqTestAttemptUncheckedUpdateInput,
      });
    ...
    console.error("eq-test/score error:", err);
    return NextResponse.json(
      { error: "Failed to score EQ test" },
      { status: 500 },
    );
  }
}
