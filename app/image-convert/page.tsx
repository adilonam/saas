...
export default function ImageConvertPage() {
  ...
  const handleConvert = async () => {
    if (status === "unauthenticated" || !session) {
      router.push(
        `/signup?callbackUrl=${encodeURIComponent(pathname || "/image-convert")}`,
      );
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError((data as { error?: string }).error || "Failed to convert image");
        return;
      }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-muted-foreground">
              Convert an image to PNG/JPEG/WebP with optional size, quality, and metadata stripping.
            </p>
          </div>
        </div>
        ...
        {resultUrl && (
          <div className="rounded-xl border border-input bg-muted/30 p-6 space-y-4">
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div>
                <p className="text-sm font-medium text-foreground">Converted image ready</p>
                {resultSize != null && (
