...
import { PhotoIcon, ArrowUpTrayIcon, ArrowDownTrayIcon, XMarkIcon } from "@heroicons/react/24/outline";
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError(errorData?.error || errorData?.detail || "Failed to extract images");
        return;
      }
      ...
      console.error("pdf-extract-images error:", err);
      setError("An error occurred while extracting images. Please try again.");
    } finally {
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        <div className="mb-8">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-lg">
            Upload a PDF and download embedded images as a ZIP.
          </p>
        </div>
        ...
        {file && (
          <div className="mb-6 p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl flex items-center justify-between">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              <PhotoIcon className="size-6 text-amber-600 dark:text-amber-400 shrink-0" />
        ...
        <div className="p-6 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl">
          ...
          <ul className="text-sm text-amber-800 dark:text-amber-200 list-disc list-inside space-y-1">
            <li>If the PDF has no embedded images, the backend may return an error.</li>
            <li>Output is a ZIP containing PNG images.</li>
          </ul>
        </div>
      </div>
    </DashboardLayout>
