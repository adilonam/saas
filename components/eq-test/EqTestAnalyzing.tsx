...
import { EQ_TESTIMONIALS } from "@/lib/eq-test/testimonials";

const ANALYSIS_STEPS = [
  {
    title: "Processing responses",
    description: "Evaluating the quality of your situational judgments",
  },
  {
    title: "Measuring EQ dimensions",
    description:
      "Analyzing self-awareness, regulation, empathy, social skill, and motivation",
  },
  {
    title: "Factoring pace",
    description: "Accounting for how thoughtfully you moved through the scenarios",
  },
  {
    title: "Computing EQ score",
    description: "Generating your emotional intelligence profile",
  },
];
...
export default function EqTestAnalyzing({
  onComplete,
}: {
    ...
    <div className="mx-auto w-full max-w-2xl space-y-8">
      ...
      <div className="rounded-3xl border border-slate-200 bg-white px-6 py-8 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {EQ_TESTIMONIALS.map((t) => (
            <div
              key={t.name}
              className="rounded-2xl border border-slate-100 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-800/50"
            >
              <div className="flex items-center gap-2">
                <div className="flex size-9 items-center justify-center rounded-full bg-teal-100 text-sm font-bold text-teal-700">
                  {t.name[0]}
                </div>
                <div>
