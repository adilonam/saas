...
/**
 * Build a complete, pdflatex-compatible IQ score report document.
 */
export function buildIqReportLatex(
  result: IqTestResult,
  user: IqReportUser,
  ...
  const accuracyExtra =
    ...
    typeof result.totalScored === "number"
      ? ` (${result.correctCount}/${result.totalScored} scored items)`
      : "";
...
\\begin{center}
  {\\LARGE\\bfseries IQ Score Report}\\\\[0.4em]
  {\\large Cognitive Profile Summary}
\\end{center}
...
\\begin{center}
  {\\Huge\\bfseries IQ ${Math.round(result.iq)}}\\\\[0.5em]
  Higher than ${Math.round(result.percentile)}\\% of the population\\\\[0.3em]
  Puzzle accuracy: ${Math.round(result.accuracy)}\\%${escapeLatex(accuracyExtra)}\\\\
  Time taken: ${elapsed}
\\end{center}
...
\\hrule
\\vspace{0.8em}
{\\small This report was generated from your completed IQ assessment. Scores are estimates based on puzzle performance and are for personal insight only.}
